...
function Navbar() {
  ...
  const handleLinkClick = (e, href) => {
    e.preventDefault();
    console.log("link which i clicked", href);
    
    const targetElement = document.querySelector(href);
    if (targetElement) {
      console.log("target found ",href);
      ...
      <nav className="fixed left-0 right-0 z-50 lg:top-4">
        <div className="mx-auto hidden max-w-xl items-center justify-center rounded-full border border-white/30 py-2 backdrop-blur-lg lg:flex">
          <div className="flex items-center justify-between gap-6">
            <div>
              <a href="/">
                <span className="uppercase text-xl">DIPIN T</span>
              </a>
            </div>
            <div>
              <ul className="flex items-center gap-4">
                {NAVIGATION_LINKS.map((item, index) => (
                  <li key={index}>
                    <a
                      className="text-sm uppercase hover:text-stone-300 "
                      href={item.href}
        ...
        <div className="py-2 backdrop-blur-md lg:hidden">
          <div className="flex items-center justify-between">
            <div>
              <a href="#">
                <span className="pl-2 text-2xl uppercase">DIPIN T</span>
              </a>
            </div>
            <div className="flex items-center">
              <button
                ...
                arua-label={isMobileMenuOpen ? "Close Menu" : "Open Menu"}
              >
                {isMobileMenuOpen ? (
          ...
          {isMobileMenuOpen && (
            <ul className="my-4 ml-4 flex flex-col gap-6 backdrop-blur-md">
                {NAVIGATION_LINKS.map((item,index)=>(
                  <li key={index}>
                    <a href={item.href} className="block w-full text-lg uppercase" onClick={(e)=> handleLinkClick(e,item.href)}>
                      {item.label}
                    </a>
                  </li>
                )
