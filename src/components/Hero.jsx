...
function Hero() {
  ...
  useEffect(()=>{
    const  ctx=gsap.context(()=>{
      const tl=gsap.timeline({
        default:{ease:"power3.out" ,duration:0.6},
      });
      tl.from(
        ".hero-title",{
        opacity:0,
  ...
  return (
    ...
    >
      ...
      <a
        href="/dipin_resume.pdf"
        target="_blank"
        rel="noopener noreferrer"
        download
        className="hero-btn mb-6 flex gap-1 rounded-full border border-pink-200/50 px-3 py-3 tradking-tighter"
      >
        <span>
            Download resume
        </span>
      ...
      <img src={avatar} alt={PROFILE.name} width={400} height="400"  className="hero-img rounded-3xl border border-purple-300/20 p-2"/>
    </section>
  );
}
